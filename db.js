...
const DB_PATH = path.join(DATA_DIR, 'lenden.db');
...
async function initDb() {
  ...
  _sqlDb = new SQL.Database(data || undefined);
...
CREATE TABLE IF NOT EXISTS accounts (
  ...
  /* Migrate: add other_income to CHECK constraint */
  ...
  /* Migrate: add quotation/sales_return/purchase_return types */
  const tcur = db.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='transactions'`).get();
  if (tcur && tcur.sql && !tcur.sql.includes('quotation')) {
    db.exec('PRAGMA foreign_keys = OFF');
    db.exec('BEGIN');
    try {
      db.exec('ALTER TABLE transactions RENAME TO transactions_old');
  ...
  _sqlDb.run(`CREATE TABLE IF NOT EXISTS reminders (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  business_id INTEGER NOT NULL,
  title TEXT NOT NULL,
  ...
  _sqlDb.run(`CREATE TABLE IF NOT EXISTS journal_entries (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  business_id INTEGER NOT NULL,
...
module.exports = { db, initDb, begin, commit, rollback, getBusiness, getUserByToken };
